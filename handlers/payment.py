...
async def cb_yoomoney(call: CallbackQuery, session: AsyncSession, bot: Bot):
    days = int(call.data.split(":")[2])
    user = await get_or_create_user(session, call.from_user.id)
    lang = user.lang or "en"
    amount = RUB_PRICES[days]
    ...
    text = (
        ...
        f"Сумма: <b>{amount}₽</b>\n\n"
        f"1. Нажмите кнопку ниже — откроется страница оплаты\n"
        f"2. Оплатите через СБП\n"
        f"3. Вернитесь сюда — статус обновится автоматически ✅"
    )

    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"💳 Оплатить {amount}₽ через СБП", url=pay_url)],
        [InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"check_payment:{pending.id}")],
        [InlineKeyboardButton(text=t("back", lang), callback_data=f"plan:{days}")],
    ])
...
async def cb_check_payment(call: CallbackQuery, session: AsyncSession, bot: Bot):
    ...
    user = await get_or_create_user(session, pending.telegram_id)
    lang = user.lang or "en"

    await call.answer("⏳ Проверяем оплату...")

    paid = await check_payment(pending.label, pending.amount)

    if paid:
        await _activate_subscription(
            session, bot, user, pending.units, lang, call.message, payment_method=pending.method
        )
        await session.delete(pending)
        await session.commit()
    else:
        await call.message.answer(
            "❌ Оплата пока не найдена. Подождите 1-2 минуты и нажмите снова.",
        )
